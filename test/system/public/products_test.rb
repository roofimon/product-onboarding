...
class PublicProductsTest < ApplicationSystemTestCase
  ...
  test "displays products in grid layout" do
    ...
    assert_selector "#products-grid"
    assert_selector "[id^='card_product_']", minimum: 1
  end
  ...
  test "product cards show name, description, seller info, and pricing" do
    ...
    within find("##{dom_id(product, :card)}") do
      assert_selector "##{dom_id(product, :name)}"
      assert_selector "##{dom_id(product, :description)}"
      assert_selector "##{dom_id(product, :seller)}"
      assert_selector "##{dom_id(product, :pricing)}"
      assert_selector "##{dom_id(product, :price_value)}"
      assert_selector "##{dom_id(product, :bid_value)}"
    end
  end
  ...
  test "product images are present or placeholder is shown" do
    ...
    product = Product.first
    within find("##{dom_id(product, :card)}") do
      assert_selector "##{dom_id(product, :image)}"
      # Either image or placeholder should be present
      has_image = has_selector?("##{dom_id(product, :image_tag)}", wait: false)
      has_placeholder = has_selector?("##{dom_id(product, :image_placeholder)}", wait: false)
  ...
  test "view details link is present on product cards" do
    ...
    product = Product.first
    within find("##{dom_id(product, :card)}") do
      assert_selector "##{dom_id(product, :view_details)}"
...
#   # Search Functionality Tests
  ...
  test "search by product name exact match" do
    ...
    fill_in "nav-search-field", with: "Vintage Watch Collection"
    find("#nav-search-field").native.send_keys(:return)
  ...
  test "search by product name partial match" do
    ...
    fill_in "nav-search-field", with: "Vintage"
    find("#nav-search-field").native.send_keys(:return)
  ...
  test "search by description" do
    ...
    fill_in "nav-search-field", with: "rare"
    find("#nav-search-field").native.send_keys(:return)
  ...
  test "search with no results shows empty state" do
    ...
    fill_in "nav-search-field", with: "Nonexistent Product XYZ"
    find("#nav-search-field").native.send_keys(:return)

    assert_selector "#empty-state"
    assert_selector "#empty-title", text: "No products found"
    assert_text "No products match your search"
    assert_selector "#clear-search-button"
  end
...
#     product_cards = all("[id^='card_product_']")
...
#       first_price = find("##{dom_id(first_product, :price_value)}").text
...
#         find("##{dom_id(p, :price_value)}").text.gsub(/[^0-9.]/, "").to_f
#       end
...
#       prices = products.map { |p| find("##{dom_id(p, :price_value)}").text.gsub(/[^0-9.]/, "").to_f }
...
#     assert_selector "#sort-select"
#     assert_selector "#sort-label", text: "Sort by:"
    
#     # Verify all sort options are present
#     select_element = find("#sort-select")
...
#     fill_in "nav-search-field", with: "Product"
#     find("#nav-search-field").native.send_keys(:return)
...
#     fill_in "nav-search-field", with: "ThisProductDoesNotExist12345"
#     find("#nav-search-field").native.send_keys(:return)
...
#     fill_in "nav-search-field", with: "Nonexistent"
#     find("#nav-search-field").native.send_keys(:return)

#     assert_selector "#clear-search-button"
#     find("#clear-search-button").click
...
#     assert_selector "#logo-link"
#     logo_link = find("#logo-link")
...
#     assert_selector "#nav-search"
#     assert_selector "#nav-search-field[placeholder='Search products...']"
#     assert_selector "#nav-search-button", text: "Search"
#   end

#   test "login button is present when not logged in" do
#     visit root_path

#     assert_selector "#nav-login-button", text: "Login"
#     assert_equal login_path, find("#nav-login-button")[:href]
#     assert_selector "#nav-actions"
#   end

#   test "navigation is visible" do
#     visit root_path

#     assert_selector "#main-nav"
#     assert_selector "#nav-content"
#   end
...
#     product_card = find("##{dom_id(product, :card)}")
#     product_card.click
...
#     assert_selector "#public-product-show-pricing-card"
#   end
